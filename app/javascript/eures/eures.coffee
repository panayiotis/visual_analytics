  ...
  constructor: (element_data) ->
    ...
    .await (error, data, geojson) =>
      if error
        ...
        @bench.push ['fetch data time',"#{(timeb-timea)/1000} seconds"]
        @bench.push ["data size", formatNumber(sizeof(@data))]
        @bench.push ["data rows", @data.rows.length]
        @bench.push ["geojson size", formatNumber(sizeof(@geojson))]
        @bench.push ["geojson features", @geojson.features.length]
        @renderBench()
